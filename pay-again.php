	...
	public function iamport_credit_card_form_fields($default_fields, $id) {
		...
		if ($this->is_first_payment()) {
			$iamoprt_fields = array(
				...
				<label for="' . esc_attr( $id ) . '-card-birth">' . __( '카드소지자확인(생년월일 또는 사업자등록번호)', 'iamport-for-woocommerce' ) . ' <span class="required">*</span></label>
				<input id="' . esc_attr( $id ) . '-card-birth" class="input-text wc-credit-card-form-card-birth" type="password" autocomplete="off" placeholder="' . esc_attr__( '생년월일 또는 사업자등록번호(법인카드)', 'iamport-for-woocommerce' ) . '" name="' . ( $args['fields_have_names'] ? $this->id . '-card-birth' : '' ) . '" maxlength="10"/>
				</p>',
